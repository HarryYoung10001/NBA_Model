...
class Config:
    ...
    # 交易机制参数
    EPSILON_TRADE = 0.1  # ε_trade: Fair Trade容忍度
    
    # 选秀机制参数
    ALPHA_DRAFT = 2.0  # α: 选秀公平性调节器
    
    # Salary Cap
    SALARY_CAP = 154_600_000  # 工资帽
    ...
    # NYK的固定偏好向量
    PV_NYK = np.array([0.505, 0.40, -0.005, 0.06, 0.04])
...
def calculate_expected_salary(as_score: float) -> float:
    """根据AS评分预测预期薪资"""
    if pd.isna(as_score):
        return 0
    ...
    return max(3766934.83, p)

def calculate_salary_efficiency(salary: float, as_score: float) -> float:
    """
    薪资惩罚函数：检测薪资是否过高
    返回-1到1之间的值：
    - 负值：薪资过高（过度支付）
    ...
    """
    # 1. 参数检查
    if pd.isna(salary) or pd.isna(as_score) or salary <= 0:
        return 0.0
    
    # 2. 计算预期薪资
    expected = calculate_expected_salary(as_score)
    
    # 3. 计算薪资差异（实际 - 预期）
    diff = salary - expected
    
    # 4. 用残差标准差标准化
    sigma = 904270.18
    z = diff / sigma
    
    # 5. 映射到(-1,1)之间，使用快速近似方法
    # 使用公式：x / (1 + abs(x)) 比tanh更快且保持符号
    penalty = z / (Config.PENALTY + abs(z))
...
def create_player_asset_from_data(row: pd.Series) -> Asset:
    ...
    # 计算派生属性
    sal_eff = calculate_salary_efficiency(salary, as_score)
